 ...
 * under the License.
 */
package eu.h2020.symbiote.rap.util;
...
public class Util {
  ...
  public static boolean entityMatchesAllKeys(EdmEntityType edmEntityType, Entity rt_entity,
    ...
    for (final UriParameter key : keyParams) {
      ...
      //handle String key types which are delimited with ' '
      if (edmKeyProperty.getType().getFullQualifiedName().equals(EdmPrimitiveTypeKind.String.getFullQualifiedName())) {
          keyText = keyText.substring(1, keyText.length()-1);
      }
      
      boolean matches = valueAsString.equals(keyText);
      //System.out.println(valueAsString + " " + keyText + " " + matches);
      if (matches) {
